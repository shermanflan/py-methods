...
def pickFiles(src, arc):
    ...
    try:
        ...
        with os.scandir(path=src) as d:
            ...
            for fi in newfiles:
                print(f'{fi}')
                #parseiJSON(filename=fi)
                parseiJSONDb(filename=fi)

            # Ok, all good -> archive.
            for fi in newfiles:
                #shutil.copy2(src=entry.path, dst=arc) # preserves file metadata
                shutil.move(src=fi, dst=arc) # preserves file metadata
...
def parseiJSON(filename=r"C:\Users\ricardogu\Desktop\test4.json"):
    ...
    try:
        ...
        print(f'parseiJSON: {e} for {filename}')
        return
...
def parseiJSONDb(filename=r"C:\Users\ricardogu\Desktop\test4.json"):
    ...
    try:
        ...
        elapsed = datetime.now() - created
        print(f'Parsed {ultipro} ultipro, {standalone} standalone customers ({elapsed})')
        ...
        print(f'parseiJSONDb: {e} for {filename}')
        raise
